...
import { RiCheckboxCircleLine, RiDeleteBack2Line } from 'react-icons/ri'
...
export function FileItem({ state, name, size, type }: FileItemProps) {
  ...
  return (
    <div className={base()}>
      <span className={icon()}>{/* <ImageIcon className="h-4 w-4" /> */}</span>
      ...
      {state === 'complete' ? (
        <RiCheckboxCircleLine className="h-5 w-5 fill-violet-600 text-white dark:fill-violet-300 dark:text-zinc-900" />
      ) : (
        <Button type="button" className={deleteButton()}>
          <RiDeleteBack2Line className="h-5 w-5" />
        </Button>
      )}
    </div>
  )
